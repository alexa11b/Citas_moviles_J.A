...
class TarjetaCita extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Card(
      ...
      color: cita.estado == EstadoCita.Cancelada ? Colors.grey[300] : Colors.white,
      child: ListTile(
        leading: Icon(
          Icons.event_available,
          color: cita.estado == EstadoCita.Cancelada ? Colors.grey : Colors.blue,
        ),
        title: Text(cita.servicioNombre, style: const TextStyle(fontWeight: FontWeight.bold)),
        subtitle: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text(esCliente ? 'Con: ${cita.servicioNombre}' : 'Cliente: ${cita.clienteNombre}'),
            Text('Fecha: ${cita.fecha.day}/${cita.fecha.month}/${cita.fecha.year} a las ${cita.hora}'),
            Text('Estado: ${cita.estado.name}', style: TextStyle(fontWeight: FontWeight.bold, color: _getColorEstado(cita.estado))),
          ],
        ),
        trailing: cita.estado == EstadoCita.Confirmada
            ? IconButton(
  ...
  Color _getColorEstado(EstadoCita estado) {
    switch (estado) {
      case EstadoCita.Confirmada: return Colors.green;
      case EstadoCita.Cancelada: return Colors.red;
      case EstadoCita.Completada: return Colors.blue;
